...
que aparece acá es una constante del código, y se indica el módulo donde vive, para
...
bien y calibran mal los números: si se les pide directamente ``ponele una nota del 1
al 5'', la misma conversación saca notas distintas en dos corridas.
\end{nota}
...
\cod{*reabierto*} & Delata un cierre que no pegó (no corta) \\
...
\medskip
Dos reglas de ajuste evitan cortes injustos:

\begin{itemize}
\item \textbf{Gracia de cierre: 120 s} (\cod{GRACIA\_CIERRE\_SEG}). El operador cierra
y adjunta el comprobante enseguida; es un solo gesto y no dos interacciones.
\item \textbf{Cola de cortesía: 600 s} (\cod{GRACIA\_CORTESIA\_SEG}). El ``gracias''
que el cliente manda después del cierre se pega hacia atrás, a la atención que se lo
ganó. Sin esto, el CRM asigna la conversación a otra persona y esa persona cobra
\est{1} sin haber escrito una palabra.
\end{itemize}

\begin{nota}
\textbf{El identificador de una interacción es determinista}: se deriva de
(sesión, instante de inicio). Pegar la cola de cortesía \emph{absorbe} fragmentos pero
nunca mueve un inicio, así que recalificar no re-numera nada.
\end{nota}
...
Esta es la parte que más se malinterpreta: \textbf{no todas las conversaciones pasan
por el mismo camino}. Hay una cascada, y la primera regla que aplica gana.

\subsection{Paso 0 --- ¿es evaluable?}

Antes de cualquier rúbrica, un filtro decide si la conversación se puede calificar.
Las que no, quedan registradas con un motivo de salteo: no desaparecen del tablero,
para que la cobertura sea auditable.

\begin{center}
\begin{tabularx}{\textwidth}{@{}lX@{}}
\toprule
\textbf{Motivo de salteo} & \textbf{Cuándo} \\
\midrule
...
\cod{customer\_media\_only} & El cliente solo mandó imágenes o audio \emph{y} el
...
\begin{alerta}
\textbf{Lo que ya NO se saltea:} que el negocio no haya contestado. Hasta el 21 de
agosto de 2026 eran 1.167 sesiones salteadas, y el salteo escondía la peor falla que
...
escribirle nunca al cliente. Eso es deliberado y atribuible, no un descuido.
\end{alerta}
...
2 & Cola de cortesía   & El fragmento es solo un ``gracias'' $\rightarrow$ se saltea \\
...
\begin{nota}
\textbf{Por qué ``sin respuesta'' va primero.} Es el único hecho que no necesita
interpretación: o hay un mensaje del negocio al cliente, o no lo hay. Y hay una
trampa: \textbf{la nota del CRM no cuenta como respuesta}. Es \cod{from\_me} pero no
es un mensaje al cliente, y contarla convertiría justo estas sesiones en ``sí
respondió''.
\end{nota}
...
Seis motivos se califican \textbf{sin LLM}: el resultado sale de hechos medibles
(hubo comprobante, hubo confirmación, cuánto tardó). Son reproducibles al cien por
cien y no cuestan inferencia.
...
\est{2} & Respondió pero \textbf{nunca confirmó} que la plata entró \\
...
ciertos pasos, y castigarlos sería exigir lo prohibido:
...
pertenece. Eso es una consulta, no un reflejo. Cobrarle el minuto sería cobrarle la
verificación que el propio manual le manda hacer.
...
El retiro tiene \textbf{dos relojes}: cuánto tardó en responder, y cuánto tardó en
...
El reloj corre desde que el cliente entrega sus datos hasta que recibe usuario y clave.
...
\est{2} & Entregó sus datos pero \textbf{nunca recibió usuario y clave}: el alta quedó a medias \\
\est{3} & Entregó las credenciales pero tardó más de 5 min \\
\est{3} & Entregó las credenciales pero no se puede medir (salieron antes de los datos) \\
...
\est{2} & Respondió después de 15 min: la consulta ya se enfrió \\
\est{3} & Respondió entre 1 y 15 min \\
\est{4} & Respondió $\le$ 1 min \\
\est{5} & Mandó el \textbf{material} de la promo y respondió $\le$ 5 min \\
\bottomrule
\end{tabularx}
\end{center}
...
\est{2} & Contestó, pero el cliente \textbf{no se llevó nada concreto}: ni un paso a
seguir ni la certeza de que se escaló \\
...
califica es \textbf{a dónde lo mandaron}, no que lo hayan mandado.
...
\textbf{Nota} & \textbf{Condición} \\
\midrule
salteo & Traspaso limpio a una línea propia y viva: no se califica \\
...
conexiones. \emph{Cuando el hecho es nuestro, no se pregunta.}
\end{nota}
...
Para los agentes que no encajan en depósito, retiro ni información se mide la
...
preguntas de hecho y el código deriva la etiqueta.
...
\midrule
\cod{resolucion} & piso   & ¿Atendió el motivo de \emph{esta} visita? \\
\cod{iniciativa} & uplift & ¿Hizo algo más allá del trámite? \\
\cod{cortesia}   & uplift & ¿Saludó, personalizó, usó el nombre? \\
\bottomrule
\end{tabularx}
\end{center}
...
No atendió \textbf{y} hubo fricción (el cliente rogando, sin respuesta) & mala (\est{1}) \\
...
limpio topaba en \est{3} y para pasar de ahí hacía falta empuje comercial. Medido
...
\est{3}. Hacerlo perfecto valía $+0{,}13$ estrellas contra no hacerlo. La escala no
...
reinsistió) no hunde la nota: topa en \est{3}.
\end{itemize}
...
la cifra de reloj de pared acusaría de negligencia a quien contestó a los seis minutos
de abrir el turno. La misma función que descuenta la noche la usan todas las rúbricas.
\end{alerta}

\subsection{Precisión}

Los tiempos se comparan \textbf{redondeados al segundo}. Los objetivos del manual
están en minutos enteros y se comparaban contra marcas de WhatsApp con milisegundos:
dos operadores perdieron dos estrellas por 365 y 312 \emph{milisegundos}.

\newpage
% =====================================================================
\section{Salteos: lo que no se califica y por qué}
...
\begin{nota}
\textbf{El salteo de cola de cortesía (v24).} Decisión del negocio, textual:
...
Este sistema tiene deudas medidas y documentadas. Ocultarlas haría el documento menos
útil.
...
\textbf{11,7 veces} más probable sacar una nota baja por usar la frase propia. No es
conducta del operador: es el detector.
...
profesional --- se califica con la misma vara comercial que un jugador. Hay una
...
dejaría al 18\% sin evaluar y perjudicaría a 25 casos concretos. Es una decisión
tomada, no un olvido.
\end{enumerate}
