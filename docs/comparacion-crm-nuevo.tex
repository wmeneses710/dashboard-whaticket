...
% =====================================================================
\section{Para qué se usa el sistema}
\label{sec:antecedente}

Antes de comparar pantallas conviene decir qué resuelve el CRM, porque de ahí sale el
peso de cada diferencia.

El sistema \textbf{centraliza en una sola cuenta las conexiones de varias aplicaciones de
mensajería} --- WhatsApp, Telegram, Facebook, Instagram y chat web --- de modo que todo
lo que escriben las personas llegue a un único lugar en vez de a cinco aplicaciones
distintas.

Sobre esas conexiones trabaja un equipo. Se crean usuarios que contestan: operadores de
\textbf{Atención al Cliente} y \textbf{operadores externos}, que pueden trabajar tanto en
la operación como en remoto. A cada uno se le asignan conversaciones.

Y las conversaciones no son todas iguales. Del otro lado puede haber:

\begin{itemize}
  \item \textbf{jugadores} ya activos,
  \item \textbf{gente nueva} que escribe por primera vez,
  \item \textbf{agentes}, que son quienes traen más jugadores.
\end{itemize}

Cada uno necesita un trato distinto, y por eso el sistema tiene que permitir
\textbf{seccionar en tres planos a la vez}: las conversaciones, las personas a atender y
el equipo de operadores. Mientras eso ocurre, el sistema \textbf{produce estadística}
para mejorar la atención y \textbf{envía encuestas de satisfacción}.

\begin{nota}
\textbf{De este uso salen los tres criterios con que se lee todo lo que sigue.}

\begin{enumerate}
  \item \textbf{Si un canal no entra, la operación de ese canal no existe.} Por eso la
        sección \ref{sec:canales} pesa más que cualquier otra.
  \item \textbf{Si no se puede seccionar, todo le llega a todos.} De ahí dependen tanto la
        cadena que define quién atiende a quién (sección \ref{sec:visibilidad}) como la
        encuesta que hoy se envía a los agentes (sección \ref{sec:csat}).
  \item \textbf{Si la asignación no queda registrada, la estadística miente.} Es el
        motivo por el que la sección \ref{sec:estados} termina en un hallazgo del ETL y
        no solo en un inventario de pantallas.
\end{enumerate}
\end{nota}
...
Departamentos           & \parcial  & Tiene 3 de las 6 pestañas de configuración \\
Segmentación de atención& \no       & Sus departamentos no se atan a conexiones \\
Estados de conversación & \no       & El nuevo no tiene ningún filtro por estado \\
Encuesta de satisfacción& \parcial  & Existe, pero solo global: no por departamento \\
\bottomrule
\end{tabularx}
\end{center}
...
Esta es la diferencia que decide si la migración es posible. No se trata de cuántas
conexiones hay configuradas en cada lado --- eso depende de la operación y del momento ---
sino de \textbf{qué tipos de canal admite cada sistema}.

El CRM actual opera hoy sobre \textbf{cuatro canales}: WhatsApp, Telegram, Facebook e
Instagram, más chat web. El CRM nuevo declara su alcance en el propio diálogo
\emph{Agregar conexión}:
...
% =====================================================================
\section{Quién atiende a quién: la segmentación}
\label{sec:visibilidad}

El antecedente (sección \ref{sec:antecedente}) fija que hay que poder seccionar en tres
planos: las conversaciones, las personas a atender y el equipo. Esta sección documenta
cómo se resuelve hoy.

\subsection{El mecanismo real: conexión, departamento, operadores}

La segmentación \textbf{no se hace clasificando contactos}. Se hace por el canal por el
que la persona escribe, en una cadena de tres eslabones:

\begin{center}
\begin{tabularx}{\textwidth}{@{}lL@{}}
\toprule
\textbf{Eslabón} & \textbf{Dónde se configura} \\
\midrule
Conexión $\rightarrow$ departamento & Pestaña \emph{Chatbot} del departamento, campo
  \textbf{Conexiones vinculadas}. Un departamento se ata a las conexiones concretas cuyas
  conversaciones va a recibir \\
Departamento $\rightarrow$ operadores & Pestaña \emph{Equipo y asignación}, con el
  interruptor \emph{miembro} por agente y el reparto automático \\
Operador $\rightarrow$ lo que ve & Permiso \emph{Ver todos} dentro de Chats, más el
  filtro \emph{Míos / Equipo} de la lista \\
\bottomrule
\end{tabularx}
\end{center}

\begin{nota}
\textbf{Esta regla ya está en uso.} No es teoría: existen conexiones dedicadas
--- \emph{JUGADORES PLATAFORMA}, \emph{AGENTES}, \emph{AGENTES VIP} --- y departamentos
que se corresponden con ellas --- \emph{Jugadores}, \emph{Jugadores VIP},
\emph{AGENTES VIP}, \emph{AGENTES OPERATIVOS}. Poner a un grupo de personas en un número
propio y atar ese número a un departamento con sus operadores \textbf{es} la forma en que
el CRM actual resuelve ``a estas personas las atiende este equipo''.
\end{nota}

\subsection{Lo que esta forma no permite}

Al estar anclada en el canal y no en la persona, la segmentación tiene un límite claro:
\textbf{no se puede reclasificar a alguien sin cambiarlo de número}. Si un jugador pasa a
VIP, o si se quiere que a un grupo concreto lo atienda solo cierto equipo, hay que moverlo
a otra conexión --- es decir, que escriba a otro número.

Las \textbf{etiquetas} de contacto, en los dos sistemas, clasifican pero \textbf{no
restringen}: marcan al contacto sin cambiar quién lo ve. Y el permiso \emph{Ver todos} es
binario: el operador ve todos los chats o solo los suyos, sin un punto intermedio por
grupo.

\subsection{El CRM nuevo no puede replicar esta cadena}

\begin{alerta}
\textbf{El primer eslabón no existe en el CRM nuevo.} Su editor de departamento tiene tres
pestañas --- Departamento, Usuarios y asignación, Resolución del chat --- y
\textbf{ninguna permite vincular conexiones}. Un departamento del CRM nuevo agrupa
operadores, pero no declara de qué canales recibe conversaciones.

\vspace{0.4em}
El segundo eslabón sí está, y es idéntico al del actual. El tercero no se pudo verificar,
porque haría falta iniciar sesión con un operador sin permisos de administrador.

\vspace{0.4em}
Sin el primer eslabón, \textbf{la segmentación que hoy sostiene la operación no se puede
reproducir}. Es un requisito a plantear ahora, mientras el producto está en construcción.
\end{alerta}
...
\subsection{El historial de asignación se descarta entero}

\begin{alerta}
Cada registro de \cod{/conversations/report} incluye un array
...
26 claves que lee el proyector, no hay columna y no hay tabla. Lo mismo ocurre con
\cod{ticket}, el objeto del ticket embebido en cada registro. \textbf{Los dos se
descartan enteros.}
\end{alerta}

Medido contra la API sobre una ventana de 24 horas:

\begin{center}
\begin{tabular}{@{}lrr@{}}
\toprule
& \textbf{Registros} & \textbf{Proporción} \\
\midrule
Registros del report en 24 h            & 430 & \\
Con \cod{participations}                & \textbf{430} & \textbf{100\,\%} \\
Con \cod{ticket}                        & \textbf{430} & \textbf{100\,\%} \\
\midrule
Con 0 tramos de asignación              & 17  & 4,0\,\% \\
Con 1 tramo                             & 324 & 75,3\,\% \\
\textbf{Con 2 o más operadores}         & \textbf{89} & \textbf{20,7\,\%} \\
\midrule
Máximo de tramos en una conversación    & \multicolumn{2}{r}{\textbf{150}} \\
\bottomrule
\end{tabular}
\end{center}

\begin{nota}
\textbf{Por qué esto importa más allá del inventario.} Ese 20,7\,\% es el mismo fenómeno
que se midió por otro camino independiente: \textbf{25.338 calificaciones (24,7\,\% del
total) pertenecen a sesiones con más de un episodio}, donde el departamento, el canal y el
...
\vspace{0.4em}
\cod{participations} contiene exactamente el dato que falta para corregirlo. Y como viene
en el \textbf{100\,\%} de los registros del report --- no solo en los tickets abiertos ---
y el report acepta rango de fechas, \textbf{el historial se puede reconstruir hacia atrás
con un relleno, no solo capturar de aquí en adelante}. La atribución histórica es
recuperable.
\end{nota}
...
\begin{itemize}
  ...
  \item \textbf{Qué ve exactamente un operador sin permisos de administrador.} El tercer
        eslabón de la cadena de segmentación (sección \ref{sec:visibilidad}) --- si al
        entrar ve solo sus chats, los de sus departamentos o todos --- requiere iniciar
        sesión con una cuenta de operador. Todo el recorrido se hizo con perfil
        Administrador en los dos sistemas.
\end{itemize}
