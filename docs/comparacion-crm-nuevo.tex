...
\toprule
\textbf{Sección} & \textbf{Ruta actual} & \textbf{Comentario} \\
\midrule
Wäbot IA            & \cod{/ai}                  & Asistente de inteligencia artificial \\
Campañas            & \cod{/campaigns}           & Envíos masivos \\
Informes            & \cod{/reports}             & Reportería operativa \\
Mensajes programados& \cod{/scheduled-messages}  & Envíos diferidos \\
Tokens              & \cod{/tokens}              & Credenciales de API \\
Documentación       & \cod{/docs}                & Ayuda en producto \\
Soporte             & ---                        & Canal de soporte del proveedor \\
\bottomrule
\end{tabularx}
\end{center}
...
\begin{enumerate}
  ...
  \item \textbf{Informes.} El CRM actual tiene una sección de reportería que el nuevo no
        tiene. Debe definirse si se reemplaza con el \cod{/dashboard} propio del nuevo o
        si hace falta construirla.

  \item \textbf{Campañas y mensajes programados.} Funcionalidad de envío que hoy existe
